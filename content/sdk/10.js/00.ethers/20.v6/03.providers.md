...
cost of a transaction in terms of gas, ensuring you have enough funds to cover the transaction cost. The method
...
crucial for cross-layer operations, ensuring you know the gas cost for moving transactions between layers. The
method returns the estimated gas as a `bigint`.
...
the cost of transferring tokens between addresses. The method returns the estimated gas as a `bigint`.
...
the cost of withdrawing tokens from one address to another. The method returns the estimated gas as a `bigint`.
